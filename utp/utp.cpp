...
#define _sin6d _in._in6d
	...
	cstr fmt(str s, size_t len) const
	{
		...
		str i;
		if (family == AF_INET) {
			_inet_ntop(family, (uint32*)&_sin4, s, len);
			i = s;
			while (*++i) {}
		} else {
			i = s;
			*i++ = '[';
			_inet_ntop(family, (in6_addr*)&_in._in6addr, i, len-1);
			while (*++i) {}
			*i++ = ']';
		}
...
struct addr_less : std::binary_function <PackedSockAddr,PackedSockAddr,bool> 
{
  bool operator() (const PackedSockAddr& x, const PackedSockAddr& y) const 
  {
    if(x._port != y._port)
      return x._port < y._port;
    return memcmp(x._sin6, y._sin6, sizeof(x._sin6)) < 0;
  }
};

typedef std::multimap<PackedSockAddr, UTPSocket*, addr_less> TSockMap;
TSockMap g_utp_sockets;
...
void UTPSocket::send_data(PacketFormat* b, size_t length, bandwidth_type_t type)
{
	...
	//if (userdata) {
		...
		if (userdata)
			...
			on_overhead(send_to_userdata, true, n, type);
	//}
						 ...
						 UTPOnOverheadProc *on_overhead)
{
	...
	if(on_overhead)
	{
		socklen_t tolen;
		SOCKADDR_STORAGE to = addr.get_sockaddr_storage(&tolen);
		on_overhead(send_to_userdata, true, len + UTP_GetUDPOverhead((const struct sockaddr *)&to, tolen), rst_overhead);
	}
...
// Create a UTP socket
UTPSocket *UTP_Create(SendToProc *send_to_proc, void *send_to_userdata, const struct sockaddr *addr, socklen_t addrlen, UTPOnOverheadProc *on_overhead)
{
	...
	g_utp_sockets.insert(TSockMap::value_type(conn->addr, conn));
					   ...
					   UTPOnOverheadProc *on_overhead)
{
	...
	if (len < sizeof(PacketFormat) && len < sizeof(PacketFormatV1)) {
		...
		if(on_overhead)
			on_overhead(send_to_userdata, false, len + UTP_GetUDPOverhead(to, tolen), bad_overhead);
		return false;
	}
	...
	//	if (conn->addr != addr)
	//		continue;
	for(TSockMap::iterator I = g_utp_sockets.find(addr); I != g_utp_sockets.end() && I->first == addr; ++I) {
		UTPSocket *conn = I->second;
		...
		if (flags == ST_RESET && (conn->conn_id_send == id || conn->conn_id_recv == id)) {
			...
			if (conn->userdata) {
				...
				on_overhead(send_to_userdata, false, len + conn->get_udp_overhead(), close_overhead);
			return true;
			...
			if (conn->userdata) {
				...
				on_overhead(send_to_userdata, false, (len - read) + conn->get_udp_overhead(), header_overhead);
			return true;
		}
	}
	...
	if (flags == ST_RESET) {
		...
		if(on_overhead)
			on_overhead(send_to_userdata, false, len + UTP_GetUDPOverhead(to, tolen), rst_overhead);
		return true;
	}
	...
	if (flags != ST_SYN) {
		...
		UTPSocket::send_rst(send_to_proc, send_to_userdata, addr, id, seq_nr, UTP_Random(), version, on_overhead);
		return true;
	}
	...
	if (incoming_proc) {
		...
		UTPSocket *conn = UTP_Create(send_to_proc, send_to_userdata, to, tolen, on_overhead);
		...
		if (conn->userdata)
			conn->func.on_overhead(conn->userdata, false, len + conn->get_udp_overhead(), header_overhead);
		else if(on_overhead)
			on_overhead(send_to_userdata, false, len + conn->get_udp_overhead(), header_overhead);

		const size_t read = UTP_ProcessIncoming(conn, buffer, len, true);
		assert(read == 0); // SYN packets shouldnt have any data

		LOG_UTPV("0x%08x: recv send connect ACK", conn);
		conn->send_ack(true);

		/* We report incomming overhead globaly, dont double report!
		incoming_proc(send_to_userdata, conn);
		...
		on_overhead(send_to_userdata, false, len + UTP_GetUDPOverhead(to, tolen), header_overhead);
	return true;
}
...
void UTP_CheckTimeouts()
{
	...
	//for (size_t i = 0; i != g_utp_sockets.GetCount(); i++) {
	//	UTPSocket *conn = g_utp_sockets[i];
	for(TSockMap::iterator I = g_utp_sockets.begin(); I != g_utp_sockets.end(); ) {
		UTPSocket *conn = I->second;
		conn->check_timeouts();
