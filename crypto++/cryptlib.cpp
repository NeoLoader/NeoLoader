...
void SimpleKeyingInterface::ThrowIfInvalidKeyLength(size_t length)
{
	...
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
...
void SimpleKeyingInterface::ThrowIfResynchronizable()
{
	if (IsResynchronizable())
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": this object requires an IV");
...
void SimpleKeyingInterface::ThrowIfInvalidIV(const byte *iv)
{
	if (!iv && IVRequirement() == UNPREDICTABLE_RANDOM_IV)
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": this object cannot use a null IV");
...
size_t SimpleKeyingInterface::ThrowIfInvalidIVLength(int size)
{
	...
	else if ((size_t)size < MinIVLength())
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": IV length " + IntToString(size) + " is less than the minimum of " + IntToString(MinIVLength()));
	...
	else if ((size_t)size > MaxIVLength())
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": IV length " + IntToString(size) + " exceeds the maximum of " + IntToString(MaxIVLength()));
...
void StreamTransformation::ProcessLastBlock(byte *outString, const byte *inString, size_t length)
{
	...
	else if (length != 0)
	{
		char Name[128] = "";
		AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": this object does't support a special last block");
...
void AuthenticatedSymmetricCipher::SpecifyDataLengths(lword headerLength, lword messageLength, lword footerLength)
{
	if (headerLength > MaxHeaderLength())
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": header length " + IntToString(headerLength) + " exceeds the maximum of " + IntToString(MaxHeaderLength()));
	...
	if (messageLength > MaxMessageLength())
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": message length " + IntToString(messageLength) + " exceeds the maximum of " + IntToString(MaxMessageLength()));
	...
	if (footerLength > MaxFooterLength())
	{
		char Name[128] = "";
		GetAlgorithm().AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": footer length " + IntToString(footerLength) + " exceeds the maximum of " + IntToString(MaxFooterLength()));
...
public:
	...
	void AlgorithmName(char* Name) const {strcat(Name, "NullRNG");}
...
public:
	...
	size_t Put2(const byte *inString, size_t length, int messageEnd, bool blocking)
	{
		...
		if (messageEnd)
		{
			...
			{
				char Name[128] = "";
				m_decryptor.AlgorithmName(Name);
				throw InvalidCiphertext(std::string(Name) + ": invalid ciphertext");
