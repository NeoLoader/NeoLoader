...
void CCM_Base::SetKeyWithoutResync(const byte *userKey, size_t keylength, const NameValuePairs &params)
{
	...
	if (blockCipher.BlockSize() != REQUIRED_BLOCKSIZE)
	{
		char Name[128] = "";
		AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": block size of underlying block cipher is not 16");
	...
	if (m_digestSize % 2 > 0 || m_digestSize < 4 || m_digestSize > 16)
	{
		char Name[128] = "";
		AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": DigestSize must be 4, 6, 8, 10, 12, 14, or 16");
...
void CCM_Base::UncheckedSpecifyDataLengths(lword headerLength, lword messageLength, lword footerLength)
{
	if (m_state != State_IVSet)
	{
		char Name[128] = "";
		AlgorithmName(Name);
		throw BadState(Name, "SpecifyDataLengths", "or after State_IVSet");
...
void CCM_Base::AuthenticateLastHeaderBlock()
{
	...
	if (m_aadLength != m_totalHeaderLength)
	{
		char Name[128] = "";
		AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": header length doesn't match that given in SpecifyDataLengths");
...
void CCM_Base::AuthenticateLastConfidentialBlock()
{
	...
	if (m_messageLength != m_totalMessageLength)
	{
		char Name[128] = "";
		AlgorithmName(Name);
		throw InvalidArgument(std::string(Name) + ": message length doesn't match that given in SpecifyDataLengths");
