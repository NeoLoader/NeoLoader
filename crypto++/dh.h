...
public:
	...
	void GeneratePublicKey(RandomNumberGenerator &rng, const byte *privateKey, byte *publicKey) const
	{
		...
		if (FIPS_140_2_ComplianceEnabled())
		{
			...
			{
				char Name[128] = ""; this->AlgorithmName(Name);
				throw SelfTestFailure(std::string(Name) + ": pairwise consistency test failed");
			}
		}
	}
	...
	static void CRYPTOPP_API StaticAlgorithmName(char* Name)
		{GroupParameters::StaticAlgorithmNamePrefix(Name); DH_Algorithm::StaticAlgorithmName(Name);}
	//std::string AlgorithmName() const {return StaticAlgorithmName();}
	void AlgorithmName(char* Name) const {StaticAlgorithmName(Name);}
