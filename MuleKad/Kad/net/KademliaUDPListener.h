...
#define DebugSend(what, ip, port)	LogKadLine(LOG_DEBUG /*logClientKadUDP*/, L" >>>    %s to %s", what, IPToStr(ip, port).c_str());
#define DebugRecv(what, ip, port)	LogKadLine(LOG_DEBUG /*logClientKadUDP*/, L"    <<< %s from %s", what, IPToStr(ip, port).c_str());
...
public:
	...
	void SendPacket(const CBuffer& data, uint8_t opcode, uint32_t destinationHost, uint16_t destinationPort, const CKadUDPKey& targetKey, const CUInt128* cryptTargetID);
...
private:
	...
	// Debug
	void DebugClientOutput(const wstring& place, uint32_t kad_ip, uint32_t port, const uint8_t* data = NULL, int len = 0);
