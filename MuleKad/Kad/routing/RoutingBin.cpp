...
bool CRoutingBin::AddContact(CContact *contact)
{
	...
	if (sameSubnets >= 2 && !IsLanIP(contact->GetIPAddress())) {
		LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Ignored kad contact (IP=%s) - too many contact with the same subnet in RoutingBin", IPToStr(contact->GetIPAddress(), contact->GetUDPPort()).c_str());
		return false;
	}
...
void CRoutingBin::AdjustGlobalTracking(uint32_t ip, bool increase)
{
	...
	if (increase) {
		if (sameIPCount >= MAX_CONTACTS_IP) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Global IP Tracking inconsistency on increase (%s)", IPToStr(ip).c_str());
			ASSERT(0);
		}
		sameIPCount++;
	} else /* if (!increase) */ {
		if (sameIPCount == 0) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Global IP Tracking inconsistency on decrease (%s)", IPToStr(ip).c_str());
			ASSERT(0);
		}
		sameIPCount--;
	}
	...
	if (increase) {
		if (sameSubnetCount >= MAX_CONTACTS_SUBNET && !IsLanIP(ip)) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Global Subnet Tracking inconsistency on increase (%s/24)",IPToStr(ip).c_str());
			ASSERT(0);
		}
		sameSubnetCount++;
	} else /* if (!increase) */ {
		if (sameSubnetCount == 0) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Global Subnet Tracking inconsistency on decrease (%s/24)",IPToStr(ip).c_str());
			ASSERT(0);
		}
		sameSubnetCount--;
	}
...
bool CRoutingBin::ChangeContactIPAddress(CContact *contact, uint32_t newIP)
{
	...
	ASSERT(GetContact(contact->GetClientID()) == contact);
	...
	if (sameIPCount >= MAX_CONTACTS_IP) {
		LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Rejected kad contact IP change on update (old IP=%s, requested IP=%s) - too many contacts with the same IP (global)"
			, IPToStr(contact->GetIPAddress()).c_str(), IPToStr(newIP).c_str());
		return false;
	}
	...
	if ((contact->GetIPAddress() & 0xFFFFFF00) != (newIP & 0xFFFFFF00)) {
		...
		if (sameSubnetGlobalCount >= MAX_CONTACTS_SUBNET && !IsLanIP(newIP)) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Rejected kad contact IP change on update (old IP=%s, requested IP=%s) - too many contacts with the same Subnet (global)"
				, IPToStr(contact->GetIPAddress()).c_str(), IPToStr(newIP).c_str());
			return false;
		}
		...
		if (sameSubnets >= 2 && !IsLanIP(newIP)) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Rejected kad contact IP change on update (old IP=%s, requested IP=%s) - too many contacts with the same Subnet (local)"
			, IPToStr(contact->GetIPAddress()).c_str(), IPToStr(newIP).c_str());
			return false;
		}
	}
	...
	// everything fine
	LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Index contact IP change allowed %s -> %s"
		, IPToStr(contact->GetIPAddress()).c_str(), IPToStr(newIP).c_str());
...
CContact *CRoutingBin::GetRandomContact(uint32_t maxType, uint32_t minKadVersion) const
{
	...
	uint32_t randomStartPos = (uint16_t)GetRand64() % m_entries.size();
	uint32_t index = 0;
...
bool CRoutingBin::CheckGlobalIPLimits(uint32_t ip, uint16_t port)
{
	...
	if (sameIPCount >= MAX_CONTACTS_IP) {
		LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Ignored kad contact (IP=%s) - too many contacts with the same IP (global)", IPToStr(ip, port).c_str());
		return false;
	}
	...
	if (sameSubnetGlobalCount >= MAX_CONTACTS_SUBNET && !IsLanIP(ip)) {
		LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Ignored kad contact (IP=%s) - too many contacts with the same subnet (global)", IPToStr(ip, port).c_str());
		return false;
	}
	return true;
}
