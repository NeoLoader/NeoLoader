...
public:
	...
	void ToByteArray(uint8_t *pby) const;

	void Write(CBuffer* data) const;
	bool Read(CBuffer* data);

	wstring ToBinaryString() const;
	wstring ToHexString() const;
	bool FromHexString(const wstring& HexString);
