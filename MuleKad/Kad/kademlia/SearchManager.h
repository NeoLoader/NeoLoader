...
typedef std::list<wstring> WordList;
...
public:
	...
	static void	GetAllSearches(SearchMap *searches) {*searches = m_searches;}
	...
	static CSearch* PrepareFindKeywords(const wstring& keyword, uint32_t searchTermsDataSize, const uint8_t *searchTermsData, uint32_t searchid, wstring& Error);
	...
	static void GetWords(const wstring& str, WordList *words, bool allowDuplicates = false);
	...
	static const wchar_t* GetInvalidKeywordChars() { return L" ()[]{}<>,._-!?:;\\/\""; }
	...
	static vector<wstring> SplitSearchWords(const wstring& str);
