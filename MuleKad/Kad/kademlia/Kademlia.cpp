...
time_t		CKademlia::m_lastFirewallCheck;
...
void CKademlia::Start(CPrefs *prefs)
{
	if (instance) {
		...
		delete prefs;
		ASSERT(instance->m_running);
		ASSERT(instance->m_prefs);
		return;
	}
	...
	LogKadLine(LOG_DEBUG /*logKadMain*/, L"Starting Kademlia");
...
void CKademlia::Stop()
{
	...
	LogKadLine(LOG_DEBUG /*logKadMain*/, L"Stopping Kademlia");
...
void CKademlia::Process()
{
	...
	if (m_lastFirewallCheck + (instance->m_prefs->GetFirewalled() ? MIN2S(15) : HR2S(1)) <= now) {
	...
	if (m_externPortLookup <= now && CUDPFirewallTester::IsFWCheckUDPRunning() && GetPrefs()->FindExternKadPort(false)) {
		...
		if (contact != NULL) {
			LogKadLine(LOG_DEBUG /*logKadPrefs*/, L"Requesting our external port from %s", IPToStr(contact->GetIPAddress()).c_str());
			DebugSend(L"Kad2Ping", contact->GetIPAddress(), contact->GetUDPPort());
			...
			LogKadLine(LOG_DEBUG /*logKadPrefs*/, L"No valid client for requesting external port available");
		}
	...
	if (m_consolidate <= now) {
		...
		if (mergedCount) {
			LogKadLine(LOG_DEBUG /*logKadRouting*/, L"Kad merged %u zones", mergedCount);
		}
	...
	if (!IsConnected() && !s_bootstrapList.empty() && (now - m_bootstrap > 15 || (GetRoutingZone()->GetNumContacts() == 0 && now - m_bootstrap >= 2))) {
		...
		m_bootstrap = now;
		LogKadLine(LOG_DEBUG /*logKadMain*/, L"Trying to bootstrap Kad from %s, Distance: %s Version: %u, %u contacts left"
			, IPToStr(contact->GetIPAddress()).c_str(), contact->GetDistance().ToHexString().c_str(), contact->GetVersion(), s_bootstrapList.size());
...
void CKademlia::ProcessPacket(const uint8_t *data, uint32_t lenData, uint32_t ip, uint16_t port, bool validReceiverKey, const CKadUDPKey& senderKey)
{
	try {
		...
		LogKadLine(LOG_DEBUG /*logKadMain*/, L"Exception on Kad ProcessPacket while processing packet (length = %u) from %s: %s", lenData, IPToStr(ip, port).c_str(), Exception.GetLine().c_str());
		throw;
	}
#ifndef _DEBUG
	catch (...) {
		ASSERT(0);
		LogKadLine(LOG_DEBUG /*logKadMain*/, L"Unhandled exception on Kad ProcessPacket");
		throw;
	}
#endif
}
...
bool CKademlia::FindIPByNodeID(CKadClientSearcher& requester, const uint8_t* nodeID)
{
	...
	if ((contact = GetRoutingZone()->GetContact(CUInt128(nodeID))) != NULL) {
		...
		if (contact->GetLastSeen() != 0 && time(NULL) - contact->GetLastSeen() < 1800) {
			requester.KadSearchIPByNodeIDResult(KCSR_SUCCEEDED, contact->GetIPAddress(), contact->GetTCPPort());
			return true;
		}
	}
...
{
	if (!CKadHandler::Instance()->UseLanMode() || !IsRunning()) {
		return false;
	}
	...
	if (m_lanModeCheck + 10 <= now) {
		...
		if (count == 0 || count > 256) {
			...
			if (GetRoutingZone()->HasOnlyLANNodes()) {
				if (!m_lanMode) {
					m_lanMode = true;
					LogKadLine(LOG_DEBUG /*logKadMain*/, L"Activating LAN mode");
				}
			} else {
				if (m_lanMode) {
					m_lanMode = false;
					LogKadLine(LOG_DEBUG /*logKadMain*/, L"Deactivating LAN mode");
				}
			}
		}
	}
...
void KadGetKeywordHash(const wstring& rstrKeyword, Kademlia::CUInt128* pKadID)
{
	byte Output[16];
	...
	string ansi_string;
	WStrToUtf8(ansi_string, rstrKeyword);
	
	//printf("Kad keyword hash: UTF8 %s\n",ansi_buffer);
	md4_hasher.CalculateDigest(Output,(const unsigned char*)ansi_string.c_str(),ansi_string.length());
