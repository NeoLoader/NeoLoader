...
public:
	...
	uint32_t GetKadPacketSent() const throw()	{ return m_totalPacketSent; }

	void	 AddFileID(const CUInt128& id)		{ m_fileIDs.push_back(id); }
	void	 PreparePacketForTags(CBuffer* packet, SFileInfo* file);
	void	 SetName(const wstring& name)		{ m_name = name; }
	const wstring&	 GetName() const throw()	{ return m_name; }
