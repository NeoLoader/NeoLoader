...
void CSearchManager::StopAllSearches()
{
	...
	for (SearchMap::iterator itSearchMap = m_searches.begin(); itSearchMap != m_searches.end(); ++itSearchMap)
...
CSearch* CSearchManager::PrepareFindKeywords(const wstring& keyword, uint32_t searchTermsDataSize, const uint8_t *searchTermsData, uint32_t searchid, wstring& Error)
{
	...
	try {
		...
		if (s->m_words.size() == 0) {
			Error = L"Search keyword too short: '" + keyword + L"', keyword must be at least 3 charachters long.";
			LogKadLine(LOG_ERROR, L"Kademlia: search keyword too short: %s", keyword.c_str());
			delete s;
			return NULL;
		}

		wstring wstrKeyword = s->m_words.front();

		LogKadLine(LOG_DEBUG /*logKadSearch*/, L"Keyword for search: %s", wstrKeyword.c_str());
		...
		if (AlreadySearchingFor(s->m_target)) {
			Error = L"Search keyword is already on search list: " + wstrKeyword;
			LogKadLine(LOG_ERROR, L"Kademlia: Search keyword is already on search list:  %s", wstrKeyword.c_str());
			delete s;
			return NULL;
		}
...
CSearch* CSearchManager::PrepareLookup(uint32_t type, bool start, const CUInt128& id)
{
	...
	try {
		...
		ASSERT(0);
		LogKadLine(LOG_DEBUG /*logKadSearch*/, L"Exception in CSearchManager::PrepareLookup");
		delete s;
		throw;
	}
...
vector<wstring> CSearchManager::SplitSearchWords(const wstring& str)
{
	vector<wstring> word_vector;
	for(wstring::size_type pos = 0;;)
	{
		wstring::size_type sep = str.find_first_of(GetInvalidKeywordChars(),pos);
		if(sep != wstring::npos)
		{
			word_vector.push_back(str.substr(pos,sep-pos));
			pos = sep+1;
		}
		else
		{
			word_vector.push_back(str.substr(pos));
			break;
		}
	}
	return word_vector;
...
void CSearchManager::GetWords(const wstring& str, WordList *words, bool allowDuplicates)
{
	vector<wstring> word_vector = SplitSearchWords(str);
	for(vector<wstring>::iterator I = word_vector.begin(); I != word_vector.end(); I++)
	{
		wstring current_word = MkLower(*I);
		string ansi_string;
		WStrToUtf8(ansi_string, current_word);
...
void CSearchManager::ProcessResponse(const CUInt128& target, uint32_t fromIP, uint16_t fromPort, ContactList *results)
{
	...
	if (s == NULL) {
		LogKadLine(LOG_DEBUG /*logKadSearch*/, L"Search either never existed or receiving late results (CSearchManager::ProcessResponse)");
		for (ContactList::const_iterator itContactList = results->begin(); itContactList != results->end(); ++itContactList)
...
void CSearchManager::ProcessResult(const CUInt128& target, const CUInt128& answer, TagPtrList *info)
{
	...
	if (s == NULL) {
		LogKadLine(LOG_DEBUG /*logKadSearch*/, L"Search either never existed or receiving late results (CSearchManager::ProcessResult)");
	} else {
...
{
	// Do a node lookup.
	LogKadLine(LOG_DEBUG /*logKadSearch*/, L"Starting NODESPECIAL Kad Search for %s", id.ToHexString().c_str());
	CSearch *search = new CSearch;
	search->SetName(L"Node Special");
...
bool CSearchManager::FindNodeFWCheckUDP()
{
	...
	CUInt128 id;
	id.FillRandom();
	LogKadLine(LOG_DEBUG /*logKadSearch*/, L"Starting NODEFWCHECKUDP Kad Search");
	CSearch *search = new CSearch;
	search->SetName(L"UDP FW Check");
