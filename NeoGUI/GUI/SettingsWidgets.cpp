...
QNICCombo::QNICCombo(QWidget* pParent)
: QComboBox(pParent)
{
	setEditable(true);
...
void QNICCombo::Update()
{
	const QVariantMap& Data = theGUI->GetInfoData();
	foreach(const QString NICName, Data["NICs"].toStringList())
	{
		if(findText(NICName) != -1)
			continue;
	...
	/*foreach(QNetworkInterface NIC, QNetworkInterface::allInterfaces())
	{
		...
		//if(!NIC.flags().testFlag(QNetworkInterface::IsUp))
		//{
		//	if (QStandardItemModel* Model = qobject_cast<QStandardItemModel*>(model()))
		//	{
		//		QModelIndex Index = Model->index(count() - 1, modelColumn(), rootModelIndex());
