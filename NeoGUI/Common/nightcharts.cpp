...
int Nightcharts::drawLegend(QPainter *painter)
{
    ...
    case Nightcharts::Horizontal:
    {
        ...
        for (int i=0;i<pieces.size();i++)
        {
            ...
            x += painter->fontMetrics().height()+2*dist;
            //if (i%2 == 0)
            {
                x = cX + 5;
                y += dist+painter->fontMetrics().height();
            }
            ...
            painter->drawText(x+painter->fontMetrics().height()+dist,y+painter->fontMetrics().height()/2+dist,label);
            x += painter->fontMetrics().width(label);
        }
        break;
    }
