...
bool CArchive::Extract(QMap<int, QIODevice*> *FileList, bool bDelete)
{
	...
	foreach(int ArcIndex, FileList->uniqueKeys())
	{
		FileProperty(ArcIndex, "Error", QVariant());
		Files.insert(ArcIndex, new CArchiveIO(FileList->value(ArcIndex), QIODevice::WriteOnly, bDelete));
	}
...
bool CArchive::Update(QMap<int, QIODevice*> *FileList, bool bDelete, int Level)
{
	...
	foreach(int ArcIndex, FileList->uniqueKeys())
	{
		Files.insert(ArcIndex, new CArchiveIO(FileList->value(ArcIndex), QIODevice::ReadOnly, bDelete));
...
SArcInfo GetArcInfo(const QString &FileName)
{
	...
	// RAR spetial case
	if(ArcInfo.ArchiveExt.indexOf(QRegExp("(rar|rev|r[0-9]{2,})", Qt::CaseInsensitive)) == 0)
	{
		if(ArcInfo.ArchiveExt.compare("rar", Qt::CaseInsensitive) == 0 || ArcInfo.ArchiveExt.compare("rev", Qt::CaseInsensitive) == 0) // is this a new naming scheme
		{
			...
			{
				int Pos1 = Pos+4;
				int Pos2 = ArcInfo.FileName.indexOf(QRegExp("[^0-9]"), Pos1);
				if(Pos2 == -1)
					Pos2 = ArcInfo.FileName.length();

				ArcInfo.FixRar = (ArcInfo.FileName.lastIndexOf(QRegExp("\\.part[0-9]+$", Qt::CaseInsensitive), -1) + 1 != Pos);
		...
		{
			Pos = ArcInfo.ArchiveExt.indexOf(QRegExp("[0-9]", Qt::CaseInsensitive));
			ArcInfo.PartNumber = ArcInfo.ArchiveExt.mid(Pos).toInt()+2; // .rar is 1 .r00 is 2, etc....
		}
		ArcInfo.ArchiveExt = "rar";
	}

	if(ArcInfo.ArchiveExt.indexOf(QRegExp("(part|)[0-9]{3,}")) == 0)
	{
...
#ifdef USE_7Z
	if(ArcInfo.ArchiveExt.indexOf(QRegExp("(rar|zip|7z)", Qt::CaseInsensitive)) != -1)
