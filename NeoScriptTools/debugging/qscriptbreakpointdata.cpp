...
//> NeoScriptTools
void QScriptBreakpointData::fromVariant(const QVariantMap& in)
{
	Q_D(QScriptBreakpointData);

	d->scriptId = in["scriptId"].toLongLong();
	d->fileName = in["fileName"].toString();
	d->lineNumber = in["lineNumber"].toInt();
	d->enabled = in["enabled"].toBool();
	d->singleShot = in["singleShot"].toBool();
	d->ignoreCount = in["ignoreCount"].toInt();
	d->condition = in["condition"].toString();
    d->data = in["data"];
	d->hitCount = in["hitCount"].toInt();
}

QVariantMap QScriptBreakpointData::toVariant() const
{
	Q_D(const QScriptBreakpointData);

	QVariantMap out;
	out["scriptId"] = d->scriptId;
    out["fileName"] = d->fileName;
    out["lineNumber"] = d->lineNumber;
    out["enabled"] = d->enabled;
    out["singleShot"] = d->singleShot;
    out["ignoreCount"] = d->ignoreCount;
    out["condition"] = d->condition;
    out["data"] = d->data;
    out["hitCount"] = d->hitCount;
    return out;
}
//< NeoScriptTools
