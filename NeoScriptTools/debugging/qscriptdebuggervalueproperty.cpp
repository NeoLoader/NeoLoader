...
*/
quint32 QScriptDebuggerValueProperty::flags() const
{
    Q_D(const QScriptDebuggerValueProperty);
    if (!d)
        return 0;
...
//> NeoScriptTools
void QScriptDebuggerValueProperty::fromVariant(const QVariantMap& in)
{
	d_ptr.reset(new QScriptDebuggerValuePropertyPrivate);
	d_ptr->name = in["name"].toString();
	d_ptr->value.fromVariant(in["value"].toMap());
	d_ptr->valueAsString = in["valueAsString"].toString();
    d_ptr->flags = in["flags"].toUInt();
    d_ptr->ref.ref();
}

QVariantMap QScriptDebuggerValueProperty::toVariant() const
{
	QVariantMap out;
	out["name"] = name();
	out["value"] = value().toVariant();
	out["valueAsString"] = valueAsString();
	out["flags"] = (quint32)flags();
    return out;
}
//< NeoScriptTools
...
QDataStream &operator>>(QDataStream &in, QScriptDebuggerValueProperty &property)
{
    ...
    property = QScriptDebuggerValueProperty(
        name, value, valueAsString, flags);
    return in;
}
