                                            ...
                                            const QScriptValue &result)
{
	...
	QScriptDebuggerValue value = QScriptDebuggerCommandExecutor::toDebuggerValue(result);
                                              ...
                                              bool hasHandler)
{
	...
	QScriptDebuggerValue value = QScriptDebuggerCommandExecutor::toDebuggerValue(exception);
    e.setScriptValue(value);
    q->event(e);
}
    ...
    const QScriptValue &value)
{
    ...
    QScriptDebuggerEvent e(QScriptDebuggerEvent::ForcedReturn,
                           scriptId, lineNumber, columnNumber);
    e.setFileName(agent->scriptData(scriptId).fileName());
	QScriptDebuggerValue value_ = QScriptDebuggerCommandExecutor::toDebuggerValue(value);
    e.setScriptValue(value_);
    q->event(e);
}
                                      ...
                                      const QString &fileName, int lineNumber)
{
	...
	//> NeoScriptTools < // The evaluation must always be asynchronous for remote debugging
...
void QScriptDebuggerBackend::doPendingEvaluate(bool postEvent)
{
	...
	QScriptDebuggerValue retret = QScriptDebuggerCommandExecutor::toDebuggerValue(ret);
...
//> NeoScriptTools
bool QScriptDebuggerBackend::isEvaluating()
{
    Q_D(QScriptDebuggerBackend);
    if (!d->agent)
        return false;
    return d->agent->isEvaluating();
}
//< NeoScriptTools
