...
public:
                        ...
                        int)
    {
        ...
        case 1: {
			...
			delta.removedProperties = result["removedProperties"].toStringList();
			foreach(const QVariant& var, result["changedProperties"].toList()) {
				QScriptDebuggerValueProperty data;
				data.fromVariant(var.toMap());
				delta.changedProperties.append(data);
			}
			foreach(const QVariant& var, result["addedProperties"].toList()) {
				QScriptDebuggerValueProperty data;
				data.fromVariant(var.toMap());
				delta.addedProperties.append(data);
			}
...
Qt::ItemFlags QScriptDebuggerLocalsModel::flags(const QModelIndex &index) const
{
    ...
    if ((index.column() == 1) && index.parent().isValid()) {
		...
		if (!(node->property.flags() & 0x00000001))
