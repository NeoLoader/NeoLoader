...
#else
	historian = new QScriptDebuggerConsoleHistorian();
#endif
...
bool QScriptDebuggerPrivate::debuggerEvent(const QScriptDebuggerEvent &event)
{
    ...
    case QScriptDebuggerEvent::Exception: {
        ...
        if (consoleWidget || errorLogWidget) {
			QString fn = QFileInfo(event.fileName()).fileName();
            if (fn.isEmpty()) {
                if (event.scriptId() != -1)
            ...
            QString msg = QString::fromLatin1("%0 at %1:%2: %3")
						  .arg(event.hasExceptionHandler() ? "Exception" : "Uncaught exception")
                          .arg(fn).arg(event.lineNumber()).arg(event.message());
            if (errorLogWidget)
...
{
#ifndef NO_QT_SCRIPT
	Q_ASSERT((options & QScriptCompletionProviderInterface::ConsoleCommandCompletion) == 0 || console != 0);
...
#else
    return new QScriptCompletionTask(contents, cursorPosition, frameIndex, this, this);
#endif
}
...
#else
	historian->historyAdd(contents);
	QScriptDebuggerCommandSchedulerFrontend frontend(this, this);
	frontend.scheduleEvaluate(stackWidget ? stackWidget->currentFrameIndex() : -1, contents, "console input");
#endif
}
...
//	//> NeoScriptTools
//	QScriptDebuggerJob *job = console->consumeInput(console->commandPrefix() + command, debugOutputWidget, this, true);
//	//< NeoScriptTools
...
void QScriptDebuggerPrivate::findCode(const QString &exp, int options)
{
	...
	//> NeoScriptTools
	int result = codeWidget->find(exp, options);
	//< NeoScriptTools
...
private:
    ...
    int m_index;
    QList<QScriptDebuggerContextInfo> m_infos;
};
...
public:
                        ...
                        int)
    {
        ...
        if (m_index == -1) {
			...
			QVariantMap delta = response.result().toMap();

            //const QList<qint64> &removed = delta.second;
            //for (int i = 0; i < removed.size(); ++i)
            //    m_debugger->scriptsModel->removeScript(removed.at(i));
			QVariantList removed = delta["removed"].toList();
			for (int i = 0; i < removed.size(); ++i)
				m_debugger->scriptsModel->removeScript(removed.at(i).toLongLong());

			//m_added = delta.first;
			QVariantList added = delta["added"].toList();
			for (int i = 0; i < added.size(); ++i)
				m_added.append(added.at(i).toLongLong());
...
public:
                        ...
                        int)
    {
		...
		QVariantList removed = delta["removed"].toList();
        for (int i = 0; i < removed.size(); ++i) {
			QScriptDebuggerLocalsModel *model = m_debugger->localsModels.take(removed.at(i).toLongLong());
            delete model;
        }
        finish();
    }
...
#ifndef NO_QT_SCRIPT
	else if (codeWidget && console && (console->currentFrameIndex() != -1)) {
...
private:
    QScriptDebuggerPrivate *m_debugger;
    int m_frameIndex;
    QScriptDebuggerContextInfo m_info;
};
...
#else
		consoleWidget->setCommandHistorian(d->historian);
		consoleWidget->message(QtDebugMsg, QString::fromLatin1("Welcome to the Script debugger.\n"));
#endif
...
void QScriptDebugger::setCodeWidget(QScriptDebuggerCodeWidgetInterface *codeWidget)
{
    ...
    if (codeWidget) {
        ...
        codeWidget->setScriptsModel(d->scriptsModel, d, d);
    ...
    if (d->findInScriptAction)
		...
		d->findInScriptAction->setEnabled((d->codeFinderWidget != 0));
    if (d->goToLineAction)
        //d->goToLineAction->setEnabled(hasScript);
		d->goToLineAction->setEnabled(true);
...
// Note: V8 does not support this at all
