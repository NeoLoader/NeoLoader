...
//> NeoScriptTools
void QScriptScriptData::fromVariant(const QVariantMap& in)
{
    if (!d_ptr) {
        d_ptr.reset(new QScriptScriptDataPrivate());
        d_ptr->ref.ref();
    }
    QScriptScriptDataPrivate *d = d_ptr.data();
	d->contents = in["contents"].toString();
	d->fileName = in["fileName"].toString(); 
	d->baseLineNumber = in["baseLineNumber"].toInt();
}

QVariantMap QScriptScriptData::toVariant() const
{
	QVariantMap out;
	const QScriptScriptDataPrivate *d = d_ptr.data();
    if (d) {
		out["contents"] = d->contents;
		out["fileName"] = d->fileName; 
		out["baseLineNumber"] = d->baseLineNumber;
    }
    return out;
}
//< NeoScriptTools
