...
static QScriptValue debuggerScriptValueToScriptValue(QScriptEngine *eng, const QScriptDebuggerValue &in)
{
    QScriptValue out = eng->newObject();
	out.setProperty(QString::fromLatin1("type"), QScriptValue(eng, (int)in.type()));
	switch (in.type()) {
    case QScriptDebuggerValue::BooleanValue:
		out.setProperty(QString::fromLatin1("value"), eng->toScriptValue(in.booleanValue()));
		break;
    case QScriptDebuggerValue::StringValue:
		out.setProperty(QString::fromLatin1("value"), eng->toScriptValue(in.stringValue()));
		break;
    case QScriptDebuggerValue::NumberValue:
		out.setProperty(QString::fromLatin1("value"), eng->toScriptValue(in.numberValue()));
		break;
    case QScriptDebuggerValue::ObjectValue:
		Q_ASSERT(in.objectId() < INT_MAX); // possile BUG
		out.setProperty(QString::fromLatin1("value"), eng->toScriptValue((qint32)in.objectId()));
		break;
    }
    return out;
}

static void debuggerScriptValueFromScriptValue(const QScriptValue &in, QScriptDebuggerValue &out)
{
	QScriptDebuggerValue::ValueType type = (QScriptDebuggerValue::ValueType)in.property(QString::fromLatin1("type")).toInt32();
	
	switch (type) {
    case QScriptDebuggerValue::BooleanValue:
		out = in.property(QString::fromLatin1("value")).toBoolean();
		break;
    case QScriptDebuggerValue::StringValue:
		out = in.property(QString::fromLatin1("value")).toString();
		break;
    case QScriptDebuggerValue::NumberValue:
		out = in.property(QString::fromLatin1("value")).toNumber();
		break;
    case QScriptDebuggerValue::ObjectValue:
		out = (qint64)in.property(QString::fromLatin1("value")).toInt32();
		break;
	default:
		out = QScriptDebuggerValue(type);
    }
}
    ...
    : q_ptr(parent)
{
	...
	qScriptRegisterMetaType<QScriptDebuggerValue>(commandEngine, debuggerScriptValueToScriptValue, debuggerScriptValueFromScriptValue);
    ...
    : d_ptr(new QScriptDebuggerConsolePrivate(this))
{
    ...
    loadScriptedCommands(scriptsPath, &tmp);
}
...
QScriptDebuggerConsoleCommandJob *QScriptDebuggerConsole::consumeInput(
    ...
    QScriptDebuggerCommandSchedulerInterface *commandScheduler, bool hidden)
//< NeoScriptTools
//QScriptDebuggerConsoleCommandJob *QScriptDebuggerConsole::consumeInput(
//    const QString &input, QScriptMessageHandlerInterface *messageHandler,
//    QScriptDebuggerCommandSchedulerInterface *commandScheduler)
    ...
    QScriptMessageHandlerInterface *messageHandler)
{
    messageHandler->message(
        QtDebugMsg,
        QString::fromLatin1(
            "Welcome to the Script debugger.\n"
